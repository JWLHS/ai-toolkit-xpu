...
export const modelArchs: ModelArch[] = [
  ...
  },
  {
    ...
    accuracyRecoveryAdapters: {
      // '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/wan22_14b_t2i_torchao_uint3.safetensors',
      '4bit + ARA': 'uint4|ostris/accuracy_recovery_adapters/wan22_14b_t2i_torchao_uint4.safetensors',
    },
  },
  {
    name: 'wan22_14b_i2v',
    ...
    accuracyRecoveryAdapters: {
      '4bit + ARA': 'uint4|ostris/accuracy_recovery_adapters/wan22_14b_i2v_torchao_uint4.safetensors',
    },
  },
  {
    name: 'wan22_5b',
  ...
  },
  {
    ...
    additionalSections: ['model.low_vram', 'model.layer_offloading'],
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/qwen_image_torchao_uint3.safetensors',
    },
  },
  {
    ...
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/qwen_image_2512_torchao_uint3.safetensors',
      '4bit + ARA': 'uint4|ostris/accuracy_recovery_adapters/qwen_image_2512_torchao_uint4.safetensors',
    },
  },
  {
    name: 'qwen_image_2',
    label: 'Qwen-Image 2.1（文生图 / 参考图）',
    group: 'image',
    gateUrl: 'https://huggingface.co/Qwen/Qwen-Image-2.1',
    defaults: {
      // 权重优先取本地 ComfyUI 目录（MODELS_PATH）里的 comfy 单文件；
      // 配置/processor 走 HF 的 Qwen/Qwen-Image-2.1。
      'config.process[0].model.name_or_path': ['Comfy-Org/Qwen-Image-2.1', defaultNameOrPath],
      'config.process[0].model.quantize': [true, false],
      'config.process[0].model.quantize_te': [true, false],
      // XPU：默认走 omni int8（没装轮子会自动回退 torchao int8）
      'config.process[0].model.qtype': ['xpu_int8', 'xpu_int8'],
      'config.process[0].model.qtype_te': ['xpu_int8', 'xpu_int8'],
      'config.process[0].model.low_vram': [false, false],
      'config.process[0].sample.sampler': ['flowmatch', 'flowmatch'],
      'config.process[0].train.noise_scheduler': ['flowmatch', 'flowmatch'],
      'config.process[0].train.timestep_type': ['weighted', 'sigmoid'],
      'config.process[0].sample.guidance_scale': [1, 4],
      'config.process[0].sample.sample_steps': [20, 25],
  ...
  {
    name: 'ming_image',
    label: 'Ming-Image 0.1 Design（MoE·实验）',
    group: 'experimental',
    defaults: {
      'config.process[0].model.name_or_path': ['Kijai/Ming-Image-ComfyUI', defaultNameOrPath],
      'config.process[0].model.quantize': [true, false],
      'config.process[0].model.quantize_te': [true, false],
      'config.process[0].model.qtype': ['xpu_int8', 'xpu_int8'],
      'config.process[0].model.qtype_te': ['xpu_int8', 'xpu_int8'],
  ...
  },
  {
    ...
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/qwen_image_edit_torchao_uint3.safetensors',
    },
  },
  {
    ...
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/qwen_image_edit_2509_torchao_uint3.safetensors',
    },
  },
  {
    ...
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/qwen_image_edit_2511_torchao_uint3.safetensors',
    },
  },
  {
    name: 'hidream',
    ...
    accuracyRecoveryAdapters: {
      '3bit + ARA': 'uint3|ostris/accuracy_recovery_adapters/hidream_i1_full_torchao_uint3.safetensors',
    },
  },
  {
    name: 'hidream_e1',
...
└── vae/
        ...
        doc: {
          ...
          description: (
            <div className="space-y-2">
              ...
              <p>
                <strong>静态视频片段</strong>: the image is held for 5 frames (2 latent frames) and routed through
  ...
  },
  {
    ...
    label: 'Krea 2 Turbo（含训练适配器）[编辑训练]',
    gateUrl: 'https://huggingface.co/krea/Krea-2-Turbo',
    group: 'experimental',
    defaults: {
      'config.process[0].model.name_or_path': ['krea/Krea-2-Turbo', defaultNameOrPath],
